# Fixed performance testing script
$Results = @()
...
foreach ($prompt in $TestPrompts) {
    ...
    try {
        ...
        $response = Invoke-RestMethod -Uri "http://localhost:9000/v1/chat/completions" -Method POST -ContentType "application/json" -Body $body -TimeoutSec 60
...
foreach ($prompt in $TestPrompts) {
    ...
    try {
        ...
        $response = Invoke-RestMethod -Uri "http://localhost:1234/v1/chat/completions" -Method POST -ContentType "application/json" -Body $body -TimeoutSec 60
...
$grouped = $Results | Group-Object Agent
foreach ($group in $grouped) {
    $successful = $group.Group | Where-Object { $_.Status -eq "Success" }
    $avgTime = if ($successful.Count -gt 0) { [math]::Round(($successful | Measure-Object -Property ResponseTime -Average).Average, 2) } else { 0 }
    $avgTokensPerSec = if ($successful.Count -gt 0) { [math]::Round(($successful | Measure-Object -Property TokensPerSec -Average).Average, 2) } else { 0 }
